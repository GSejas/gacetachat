...
temperature = st.sidebar.number_input("Temperature", min_value=0.0, max_value=1.0, value=0.7, step=0.01)
    ...
    try:
        ...
        for log in logs:
            ...
            if st.button("Re-Run Prompt", key=f"re-run-prompt-{log['prompt_id']}"):
                ...
                prompt_execution_engine.re_execute_prompt(session_id, log['prompt_id'], model=model, temp=temperature, max_tokens=maxtokens)
                st.rerun()
...
def run_execution_template(session, template_id, gaceta_id):
    ...
    try:
        prompt_execution_engine.execute_content_template_prompts(None, template_id, gaceta_id=gaceta_id, re_execute=False)
