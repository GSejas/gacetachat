...
from models import ExecutionSession, Prompt, ExecutionState, PromptQueryResponse, ContentExecutionLog
...
def test_run_prompt_by_date(prompt_execution_engine):
    with patch('prompt_execution_engine.FAISSHelper') as mock_faiss_helper, \
         patch('prompt_execution_engine.get_llm') as mock_get_llm, \
         patch('prompt_execution_engine.query_folder') as mock_query_folder:
...
def test_execute_prompt(prompt_execution_engine):
    with patch.object(prompt_execution_engine, 'get_execution_session_prompts_results') as mock_get_results, \
         patch.object(prompt_execution_engine, 'run_prompt_by_date') as mock_run_prompt:
        ...
        result = prompt_execution_engine.execute_prompt('query with {{alias1}}', 'session_id')
...
def test_execute_content_template_prompts(prompt_execution_engine, db_session):
    with patch.object(prompt_execution_engine, 'create_execution_session') as mock_create_session, \
         patch.object(prompt_execution_engine, 'execute_prompt') as mock_execute_prompt, \
        ...
        session_id = prompt_execution_engine.execute_content_template_prompts(1, 1, gaceta_id=1)
...
def test_re_execute_prompt(prompt_execution_engine, db_session):
    with patch.object(prompt_execution_engine, 'execute_prompt') as mock_execute_prompt, \
         patch.object(db_session, 'query') as mock_query:
...
def test_get_execution_session_prompts_results(prompt_execution_engine, db_session):
    ...
    results = prompt_execution_engine.get_execution_session_prompts_results('session_id')
...
def test_log_prompt_execution(prompt_execution_engine, db_session):
    log_entry = prompt_execution_engine.log_prompt_execution(1, 1, ExecutionState.EXECUTED.value)
