...
from models import GacetaPDF, ExecutionSession, ExecutionState
...
def mock_ai_service(mocker):
    ...
    mocker.patch('qa.query_folder', return_value={
...
def test_integration_check_and_download_today_pdf(db_session, mock_ai_service):
    ...
    exec_session = db_session.query(ExecutionSession).filter_by(document_id=gaceta.id).first()
