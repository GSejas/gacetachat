...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🌟 CRUD Operations - AI Prompt Execution Engine & Database Operations
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 Description:
    Core CRUD operations and AI prompt execution engine for GacetaChat. Handles
    database operations, prompt template processing, and AI query execution with
    FAISS vector search integration. Central orchestrator for the AI pipeline.

🏗️ Architecture Flow:
    ```
    ┌─────────────────┐    SQL/ORM    ┌──────────────────┐
    │   Frontend UI   │ ──────────────▶│  CRUD Operations │
    │   (Streamlit)   │     REST API   │  (This Module)   │
    └─────────────────┘                └──────────────────┘
            │                                    │
            │ User Queries                       │ AI Processing
            ▼                                    ▼
    ┌─────────────────┐                 ┌──────────────────┐
    │   Session Mgmt  │                 │ PromptExecution  │
    │   (SQLite DB)   │                 │ Engine (LangChain)│
    └─────────────────┘                 └──────────────────┘
                                               │
                                               │ Vector Search
                                               ▼
...
📥 Inputs:
    • Database sessions: SQLAlchemy session objects for DB operations
    • Prompt templates: Template objects with {{alias}} substitution support
    • Execution sessions: UUID-based session tracking for multi-prompt workflows
    • User queries: Natural language queries for AI processing
    • Configuration: OpenAI API keys, model settings, temperature controls

📤 Outputs:
    • Query responses: AI-generated answers with source attribution
    • Execution logs: Comprehensive audit trail with state tracking
    • Session status: INIT → EXECUTED → APPROVED/FAILED state progression
    • Prompt results: Structured JSON with answer, sources, and metadata
    • Database records: Persisted execution history and user interactions

🔗 Dependencies:
    • fastapi: HTTP exception handling and API integration
    • sqlalchemy: ORM operations and database session management
    • langchain: Prompt templating and AI chain orchestration
    • openai: GPT model integration via LangChain wrappers
    • faiss_helper: Vector similarity search and document retrieval
    • models: Database schema definitions and SQLAlchemy models
    • qa: LLM initialization and query processing pipeline

🏛️ Component Relationships:
    ```mermaid
    graph TD
        A[CRUD Module] --> B[ExecutionSession]
        A --> C[ContentExecutionLog]
        A --> D[PromptQueryResponse]
        A --> E[FAISSHelper]
        A --> F[OpenAI API]

        G[Frontend] --> A
        A --> H[(SQLite DB)]
        A --> I[LangChain Pipeline]
        I --> F
        E --> J[FAISS Index Files]

        classDef crudModule fill:#e1f5fe
        classDef database fill:#f3e5f5
        classDef external fill:#fff3e0

        class A crudModule
        class B,C,D,H database
        class F,G,I,J external
    ```

🔒 Security Considerations:
    ⚠️  HIGH: OpenAI API key exposure in config - ensure secure environment variables
    ⚠️  HIGH: SQL injection via dynamic query construction - use parameterized queries
    ⚠️  MEDIUM: Prompt injection through user input - sanitize template substitution
    ⚠️  MEDIUM: Execution session hijacking - validate session ownership
    ⚠️  LOW: Resource exhaustion via large prompts - implement token limits

🛡️ Risk Analysis:
    • API Key Security: Keys stored in environment variables, rotation needed
    • Data Privacy: User queries logged permanently, GDPR compliance required
    • Resource Limits: No rate limiting on AI API calls, cost control needed
    • Error Exposure: Stack traces may leak system information to users
    • Session Security: UUID sessions not tied to user authentication

⚡ Performance Characteristics:
    • Time Complexity: O(n*log(n)) for FAISS similarity search operations
    • Memory Usage: ~100MB baseline + vector index size (varies by date)
    • Scalability: Limited by OpenAI API rate limits (3,500 RPM)
    • Database: SQLite performance degrades with concurrent writes >10/sec

🧪 Testing Strategy:
    • Unit Tests: Individual CRUD functions with mocked dependencies
    • Integration Tests: End-to-end prompt execution with test databases
    • Performance Tests: Large document processing and concurrent sessions
    • Security Tests: Prompt injection and SQL injection vulnerability scans

📊 Monitoring & Observability:
    • Metrics: Execution success rate, response times, token usage
    • Logging: Structured logging with session IDs and prompt tracking
    • Alerts: Failed executions, API rate limit breaches, DB connection issues
    • Tracing: Request correlation across FastAPI → CRUD → AI pipeline

🔄 Data Flow:
    ```
    User Query ──▶ Session Creation ──▶ Prompt Processing ──▶ AI Response
         │               │                    │                 │
         ▼               ▼                    ▼                 ▼
    Input Validation  DB Logging      FAISS Search      Response Storage
    ```
